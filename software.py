...
class BankUi:
    def __init__(self,root):
        self.root = root
        self.header = Label(self.root,text="RamRaju BANK",bg="#0019fc",fg="white",font=("arial",20,"bold"))
    ...
    def verify_user(self):
        try:
            # Read the CSV file
             data = pd.read_csv('bank_details.csv')
             # Print columns to verify the column names
             print("Columns in the CSV file:", data.columns.tolist())
             # Ensure 'unique_id' column exists
             if 'unique_id' not in data.columns or 'password' not in data.columns:
                messagebox.showerror("Error", "The 'unique_id' or 'password' column is missing from the CSV file.")
                return
            # Filter the data based on unique_id
             filtered_data = data[data['unique_id'] == self.real_user]
             # Check if the filtered data is empty
             if filtered_data.empty:
                 messagebox.showerror("Error", f"No user found with unique_id: {self.real_user}")
                 return
             # Get the password
             self.gottenpassword = filtered_data['password'].values[0]
             # Debugging output
             print(f"Gotten password: {self.gottenpassword}")
             print(f"Entered password: {self.givenpentry.get()}")
             # Verify the entered password
             if str(self.givenpentry.get()) == str(self.gottenpassword):
                 messagebox.showinfo("Verification Info", "Verification Successful!")
                 self.final_page()
             else:
                 messagebox.showinfo("Verification Info", "Verification Failed")
                 self.begin_page()
    ...
    def user_balance(self):
        ...
        self.label = Label(self.frame, text= 'Current Account Balance: ' + 'Rs.' + str(text),font=ARIAL)
    ...
    def get_embeddings(self):
        ...
        embedder = cv2.dnn.readNetFromTorch(r'C:\\Users\\mrbha\\OneDrive\\Desktop\\RamRaju Bank mini\\nn4.small2.v1.t7')
